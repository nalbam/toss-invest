...
- `trading_audit` — §6 감사 로그 `{at, kind, decision, reasons, account_seq, symbol, order_id, notional_krw, high_value, entry}`. 모든 주문 시도(SEND/DRY_RUN/BLOCK)를 secret-free 요약으로 영속(`lib/server/trading/audit-store.ts`).
