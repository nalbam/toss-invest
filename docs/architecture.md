...
- `candle_coverage` — 심볼/인터벌별 **실제 Toss 페치가 증명한 최신 앵커 연속 구간** `{symbol, interval, covered_from_epoch, covered_to_epoch, updated_at}`, PK `(symbol, interval)`. `candle_cache`만으로는 진짜 데이터 구멍과 장중 휴장 갭을 구분할 수 없어, 이 범위 안에서만 캐시를 신뢰하고 벗어나면 라이브 페치로 폴백한다(`recordCoverageFetch`가 커서 기준으로 병합/치환, `putConfirmedCandles`에서는 절대 갱신하지 않음).
