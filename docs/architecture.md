...
- `app_settings` — UI 상태 KV `{key, value, updated_at}`, PK `key`. 테마·카드 접힘·차트 인터벌/오버레이·선택 계좌·계좌별 마지막 종목·주문 폼 설정·심볼별 주문 수량/금액·어드바이저 자동/결과 캐시 등 과거 브라우저 localStorage에 있던 모든 상태를 전역(사용자 구분 없음) 1벌로 보관. 클라이언트 `app/_components/settingsStore.ts`가 부팅 시 `GET /api/settings`로 한 번 적재해 동기 캐시로 읽고, 변경은 디바운스 batch `PUT /api/settings`(`{upserts, deletes}`)로 영속한다.
